...
import {
  ...
  ExternalLink, CreditCard, AlertTriangle, Video, Percent,
} from "lucide-react";
...
const PLANS = [
  {
    id: "basic" as const,
    label: "Basic Publisher",
    price: "€499",
    period: "/ month",
    description: "For publishers starting to curate shoppable content",
    commission: "2%",
    videoLimit: 5,
    playlistLimit: 5,
    features: [
      "5 shoppable videos per month",
      "Carousel playlists up to 5 videos",
      "2% affiliate commission",
      "Global Video Library access",
      "Creator wishlist & alerts",
      "Full video player customisation",
      "Analytics — views & sales per video",
      "API key or manual product sync",
      "Overage billed at standard rates",
    ],
  },
  {
    id: "volume" as const,
    label: "Volume Publisher",
    price: "€750",
    period: "/ month",
    description: "For publishers scaling their video commerce presence",
    popular: true,
    commission: "2.5%",
    videoLimit: 10,
    playlistLimit: 10,
    features: [
      "10 shoppable videos per month",
      "Carousel playlists up to 10 videos",
      "2.5% affiliate commission",
      "Global Video Library access",
      "Creator wishlist & alerts",
      "Full video player customisation",
      "Advanced analytics — views & sales",
      "API key or manual product sync",
      "Overage billed at standard rates",
    ],
  },
  {
    ...
    label: "Enterprise Publisher",
    price: "€1,299",
    period: "/ month",
    description: "For high-volume publishers with broad consumer audiences",
    commission: "3.5%",
    videoLimit: 30,
    playlistLimit: 30,
    features: [
      "Up to 30 shoppable videos per month",
      "Unlimited playlist videos",
      "3.5% affiliate commission",
      "Global Video Library access",
      "Creator wishlist & priority alerts",
      "Full video player customisation",
      "Premium analytics — views & sales",
      "API key or manual product sync",
...
export default function PublisherSettingsSubscription() {
  ...
  const { data: sub, isLoading } = useQuery<BrandSubscription | null>({
    queryKey: ["/api/affiliate/subscription"],
  });

  const checkoutMut = useMutation({
    mutationFn: async (plan: "basic" | "volume" | "enterprise") => {
      const res = await apiRequest("POST", "/api/affiliate/subscription/checkout", { plan });
  ...
  const portalMut = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/affiliate/subscription/portal", {});
  ...
  const surplusMut = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/affiliate/subscription/surplus-invoice", {
        views, minutes, totalAmount: totalSurplus,
      });
  ...
  const viewsCost    = views   * RATE_PER_VIEW;
  const minutesCost  = minutes * RATE_PER_MINUTE;
  const totalSurplus = viewsCost + minutesCost;

  const currentPlan = PLANS.find(p => p.id === (sub?.plan ?? "basic")) ?? PLANS[0];
  ...
  return (
    <div className="space-y-6 max-w-2xl pb-12">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Publisher Subscription</h1>
      ...
      {isSuccess && (
        <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl text-green-800 dark:text-green-300 text-sm">
          ...
          Subscription activated — you're all set to start publishing!
        </div>
      )}
      {isCancelled && (
      ...
      <Card className="border-primary/30 bg-primary/5">
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-sm">Earn affiliate commission on every sale</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Basic 2% · Volume 2.5% · Enterprise 3.5% — paid automatically via Stripe Connect
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between gap-4">
              <div>
                <CardTitle className="text-lg">{currentPlan.label}</CardTitle>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="p-3 rounded-lg bg-muted/50">
                <p className="text-lg font-bold text-primary">{currentPlan.commission}</p>
                <p className="text-xs text-muted-foreground">Commission</p>
              </div>
              <div className="p-3 rounded-lg bg-muted/50">
                <p className="text-lg font-bold">{currentPlan.videoLimit}</p>
                <p className="text-xs text-muted-foreground">Videos / mo</p>
              </div>
              <div className="p-3 rounded-lg bg-muted/50">
                <p className="text-lg font-bold">{currentPlan.playlistLimit}</p>
                <p className="text-xs text-muted-foreground">Playlist size</p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-5">
              ...
              <p className="text-xs text-muted-foreground -mt-3">
                Usage beyond your plan is billed at <strong>€0.05 / view</strong> and <strong>€0.15 / minute</strong> of playback consumed by your viewers.
              </p>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground text-right">
                  {views.toLocaleString()} × €{RATE_PER_VIEW.toFixed(2)} = <span className="font-semibold text-foreground">€{fmt(viewsCost)}</span>
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground text-right">
                  {minutes.toLocaleString()} min × €{RATE_PER_MINUTE.toFixed(2)} = <span className="font-semibold text-foreground">€{fmt(minutesCost)}</span>
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border">
                ...
                >
                  ...
                  {surplusMut.isPending ? "Creating…" : "Pay Overage"}
                </Button>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {sub?.plan === "enterprise" ? "Manage Plan" : "Upgrade Plan"}
              </Button>
              <Button
      ...
      <Dialog open={planDialogOpen} onOpenChange={setPlanDialogOpen}>
        <DialogContent className="max-w-2xl rounded-3xl">
          <DialogHeader>
            <DialogTitle>Choose your Publisher plan</DialogTitle>
            <DialogDescription>
              Taken to Stripe to complete payment securely. Cancel anytime from the billing portal.
            </DialogDescription>
          </DialogHeader>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs text-primary font-medium">
                    ...
                    {plan.commission} commission · {plan.videoLimit} videos
                  </div>
